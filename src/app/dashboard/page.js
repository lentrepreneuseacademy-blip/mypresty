...
export default function Dashboard() {
  ...
  const [newGiftCard, setNewGiftCard] = useState({ recipient_name:'',recipient_email:'',amount:50,message:'' });
  ...
  const [caisseDate, setCaisseDate] = useState('today');
  ...
  const [objectiveVal, setObjectiveVal] = useState(0);
  ...
  async function checkUser() {
    ...
    if (salons && salons.length > 0) {
      const s = salons[0];
      // Trial fallback: if no subscription_status, compute from created_at
      if (!s.subscription_status || s.subscription_status === 'loading') {
        s.subscription_status = 'trial';
      }
      if (!s.trial_start && s.created_at) {
        s.trial_start = s.created_at;
      }
      if (!s.trial_end && s.created_at) {
        s.trial_end = new Date(new Date(s.created_at).getTime() + 90 * 24 * 60 * 60 * 1000).toISOString();
      }
      setSalon(s);
      setDynPricing(s.dyn_pricing_enabled || false);
      loadData(s.id);
  ...
  async function loadData(salonId) {
    const [a,c,s,t,p,m,l,r,pr,pk,w,gc] = await Promise.all([
      ...
      supabase.from('gift_cards').select('*').eq('salon_id',salonId).order('created_at',{ascending:false}),
    ]);
    ...
    setReviews(r.data||[]);setProducts(pr.data||[]);setPackages(pk.data||[]);setWaitlist(w.data||[]);setGiftCards(gc.data||[]);
  }
  ...
  function getNotifications() {
    ...
    if(todayAppts.length>0) notifs.push({icon:'📅',text:`${todayAppts.length} RDV aujourd'hui`,time:'Aujourd\'hui'});
    getDangerClients().forEach(c=>notifs.push({icon:'🔴',text:`${c.first_name} ${c.last_name} — risque de perte`,time:'Action'}));
    products.filter(p=>p.stock<6).forEach(p=>notifs.push({icon:'⚠️',text:`Stock bas : ${p.name} (${p.stock})`,time:'Stock'}));
    reviews.filter(r=>!r.reply).slice(0,2).forEach(r=>notifs.push({icon:'⭐',text:`Avis sans réponse de ${r.client_name}`,time:'Avis'}));
    if(waitlist.length>0) notifs.push({icon:'📋',text:`${waitlist.length} en liste d'attente`,time:'Waitlist'});
    team.filter(m=>m.target_monthly>0&&(m.current_month_ca||0)>=m.target_monthly).forEach(m=>notifs.push({icon:'🎉',text:`${m.name} a atteint son objectif !`,time:'Équipe'}));
    if(salon?.subscription_status==='trial'&&salon?.trial_end){const d=Math.ceil((new Date(salon.trial_end).getTime()-Date.now())/(1000*60*60*24));if(d<=14)notifs.push({icon:'⏰',text:`Essai gratuit : ${d}j restants`,time:'Abo'});}
    return notifs;
  }

  function getAgendaSlots() {
    const dayAppts = appointments.filter(a=>a.date===agendaDate);
    const hours = [];
    for(let h=9;h<=19;h++){
      const hStr = `${h.toString().padStart(2,'0')}:00`;
      const hStr30 = `${h.toString().padStart(2,'0')}:30`;
      const apptH = dayAppts.filter(a=>a.time&&a.time.startsWith(h.toString().padStart(2,'0')));
      hours.push({time:hStr,appts:apptH.filter(a=>a.time===hStr||(!a.time?.includes(':30')&&a.time?.startsWith(h.toString().padStart(2,'0'))))});
      hours.push({time:hStr30,appts:apptH.filter(a=>a.time===hStr30)});
    }
    return hours;
  }
  ...
  function getCaisseData() {
    const now = new Date();
    const filtered = appointments.filter(a=>{const d=new Date(a.date);if(caisseDate==='today')return d.toDateString()===now.toDateString();if(caisseDate==='week'){const w=new Date(now);w.setDate(w.getDate()-7);return d>=w;}return d.getMonth()===now.getMonth()&&d.getFullYear()===now.getFullYear();}).filter(a=>a.status==='confirmed');
    const totalCA = filtered.reduce((s,a)=>s+(Number(a.price)||0),0);
    return { appointments: filtered, totalCA, total: totalCA, count: filtered.length };
  }

  function getOnboardingSteps() {
    return [{label:'Créer ton salon',done:!!salon},{label:'Ajouter une prestation',done:services.length>0},{label:'Ajouter ton équipe',done:team.length>0},{label:'Ajouter une photo',done:photos.length>0},{label:'Écrire ta description',done:!!salon?.description},{label:'Définir tes horaires',done:!!salon?.opening_time},{label:'Partager ton lien',done:true}];
  }
  function getOnboardingProgress() { const s=getOnboardingSteps();return Math.round(s.filter(x=>x.done).length/s.length*100); }

  async function saveObjective() { await saveSalonSettings({monthly_objective:objectiveVal}); setEditObjective(false); }
  ...
  // ═══════ STYLES ═══════
  const sty = { fontFamily: sf, fontSize: isMobile?13:14, letterSpacing: 2, textTransform: 'uppercase', color: '#999', marginBottom: 8 };
  const card = { background: '#FFF', border: '1px solid #E8E8E4', padding: isMobile?'16px':'22px', marginBottom: 12 };
  const inp = { width:'100%', padding:'12px 14px', border:'1px solid #E8E8E4', fontFamily:sf, fontSize:16, outline:'none', background:'#FFF', marginBottom:8, boxSizing:'border-box' };
  const btn = (outline) => ({ padding:'10px 20px', background:outline?'transparent':tc, color:outline?tc:'#FFF', border:outline?`1px solid ${tc}`:'none', fontFamily:sf, fontSize:15, fontWeight:600, letterSpacing:1.5, textTransform:'uppercase', cursor:'pointer' });
    ...
    <div style={{ fontFamily: sf, background: '#FAFAF8', minHeight: '100vh' }}>
      ...
      <main style={{padding:isMobile?'14px':'24px 32px',maxWidth:960,margin:'0 auto'}}>
        ...
        {showNotifs&&<div style={{background:'#FFF',border:'1px solid #E8E8E4',padding:'16px',marginBottom:14,boxShadow:'0 4px 20px rgba(0,0,0,0.08)'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}><p style={{fontFamily:sf,fontSize:17,fontWeight:600}}>🔔 Notifications</p><button onClick={()=>setShowNotifs(false)} style={{fontFamily:sf,fontSize:16,color:'#999',background:'none',border:'none',cursor:'pointer'}}>✕</button></div>
          {getNotifications().length===0?<p style={{fontFamily:sf,fontSize:16,color:'#999'}}>Aucune notification</p>:
          getNotifications().map((n,i)=>(<div key={i} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 0',borderBottom:i<getNotifications().length-1?'1px solid #F0F0EC':'none'}}>
            <span style={{fontSize:22}}>{n.icon}</span>
            <div style={{flex:1}}><p style={{fontFamily:sf,fontSize:16}}>{n.text}</p><p style={{fontFamily:sf,fontSize:15,color:'#BBB'}}>{n.time}</p></div>
          </div>))}
        </div>}

        {/* ONBOARDING */}
        {getOnboardingProgress()<100&&tab==='accueil'&&<div style={{background:tc,color:'#FFF',padding:'18px',marginBottom:14}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
            <p style={{fontFamily:sf,fontSize:16,fontWeight:600}}>🚀 Configure ton salon</p>
            <span style={{fontFamily:sf,fontSize:16,fontWeight:600}}>{getOnboardingProgress()}%</span>
          </div>
          <div style={{background:'rgba(255,255,255,0.2)',height:8,borderRadius:4,marginBottom:10}}><div style={{background:'#4ADE80',height:8,borderRadius:4,width:`${getOnboardingProgress()}%`,transition:'width 0.3s'}}/></div>
          {getOnboardingSteps().map((s,i)=>(<div key={i} style={{display:'flex',alignItems:'center',gap:8,padding:'4px 0'}}>
            <span style={{fontSize:16}}>{s.done?'✅':'⬜'}</span>
            <span style={{fontFamily:sf,fontSize:16,opacity:s.done?0.5:1,textDecoration:s.done?'line-through':'none'}}>{s.label}</span>
          </div>))}
        </div>}
        ...
        {/* ═══ ACCUEIL ═══ */}
        {tab === 'accueil' && (<div>
          <h1 style={{fontFamily:ss,fontSize:isMobile?30:42,fontWeight:300,marginBottom:4}}>Bienvenue, {salon.name} ✦</h1>
          <p style={{fontFamily:sf,fontSize:16,color:'#999',marginBottom:14}}>{filtered.length} RDV · {clients.length} clients · {services.length} prestations {avgRating>0&&`· ⭐ ${avgRating}`}</p>

          {/* ESSAI EN COURS */}
          {salon.subscription_status==='trial'&&salon.trial_end&&(()=>{
            const daysLeft=Math.max(0,Math.ceil((new Date(salon.trial_end).getTime()-Date.now())/(1000*60*60*24)));
            const endDate=new Date(salon.trial_end).toLocaleDateString('fr-FR',{day:'numeric',month:'long',year:'numeric'});
            return <div style={{background:'linear-gradient(135deg,#1A1A1A,#333)',color:'#FFF',padding:'20px',marginBottom:14,borderRadius:0}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
                <p style={{fontFamily:sf,fontSize:18,fontWeight:600}}>✦ Essai gratuit en cours</p>
                <span style={{fontFamily:sf,fontSize:15,background:daysLeft>30?'rgba(74,222,128,0.25)':daysLeft>7?'rgba(245,158,11,0.3)':'rgba(239,68,68,0.4)',padding:'5px 14px',borderRadius:20,fontWeight:600}}>{daysLeft} jours restants</span>
              </div>
              <p style={{fontFamily:sf,fontSize:16,opacity:0.7}}>Toutes les fonctionnalités sont débloquées jusqu'au <strong style={{color:'#FFF',opacity:1}}>{endDate}</strong></p>
              <div style={{background:'rgba(255,255,255,0.15)',height:8,borderRadius:4,marginTop:12}}><div style={{background:daysLeft>30?'#4ADE80':daysLeft>7?'#F59E0B':'#EF4444',height:8,borderRadius:4,width:`${Math.max(3,(daysLeft/90)*100)}%`,transition:'width 0.3s'}}/></div>
            </div>;
          })()}

          {dangerClients.length>0&&<div style={{background:'#FEF2F2',border:'1px solid #FEE2E2',padding:'12px 14px',marginBottom:12}}>
            <p style={{fontFamily:sf,fontSize:16,fontWeight:600,color:'#EF4444',marginBottom:6}}>🔴 {dangerClients.length} cliente(s) en danger de perte</p>
            {dangerClients.slice(0,3).map(c=>(<div key={c.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'3px 0'}}>
              <span style={{fontFamily:sf,fontSize:16}}>{c.first_name} {c.last_name} — {c.total_visits} visites</span>
              <span onClick={()=>{setTab('clients');setSelClient(c);}} style={{fontFamily:sf,fontSize:15,color:tc,fontWeight:600,cursor:'pointer'}}>Voir fiche →</span>
            </div>))}
          </div>}

          {waitlist.length>0&&<div style={{background:'#FFFBEB',border:'1px solid #FEF3C7',padding:'12px 14px',marginBottom:12}}>
            <p style={{fontFamily:sf,fontSize:16,fontWeight:600,color:'#92400E'}}>⚡ {waitlist.length} cliente(s) en liste d'attente</p>
          </div>}

          <div style={{display:'flex',gap:4,marginBottom:12}}>
            {[{id:'day',l:'Jour'},{id:'week',l:'Sem.'},{id:'month',l:'Mois'},{id:'year',l:'Année'}].map(p=>(
              <button key={p.id} onClick={()=>setStatsPeriod(p.id)} style={{padding:'6px 12px',border:statsPeriod===p.id?'none':'1px solid #E8E8E4',background:statsPeriod===p.id?tc:'#FFF',color:statsPeriod===p.id?'#FFF':'#777',fontFamily:sf,fontSize:15,letterSpacing:1,textTransform:'uppercase',cursor:'pointer'}}>{p.l}</button>
            ))}
          </div>

          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6,marginBottom:12}}>
            <div style={{background:tc,color:'#FFF',padding:'16px 14px',gridColumn:'span 2'}}>
              <p style={{fontFamily:sf,fontSize:15,textTransform:'uppercase',letterSpacing:2,opacity:0.5,marginBottom:4}}>Chiffre d'affaires</p>
              <p style={{fontSize:42,fontWeight:300}}>{caVal.toLocaleString('fr-FR')}€</p>
            </div>
            {[{l:'RDV',v:filtered.length},{l:'Panier moyen',v:filtered.length>0?Math.round(caVal/filtered.length)+'€':'—'},{l:'Clients',v:clients.length},{l:'Produits vendus',v:products.reduce((s,p)=>s+p.stock,0)}].map((c,i)=>(
              <div key={i} style={{background:'#FFF',border:'1px solid #E8E8E4',padding:'14px'}}>
                <p style={{fontFamily:sf,fontSize:15,textTransform:'uppercase',letterSpacing:2,color:'#999',marginBottom:4}}>{c.l}</p>
                <p style={{fontSize:30,fontWeight:300}}>{c.v}</p>
              </div>
            ))}
          </div>
          ...
          <div style={card}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:8}}>
              <div style={{flex:1}}><p style={{fontFamily:sf,fontSize:16,color:'#999'}}>Confirmé</p><p style={{fontFamily:sf,fontSize:26,fontWeight:300}}>{prevCA.confirmed}€</p></div>
              <div style={{width:1,height:30,background:'#E8E8E4'}}/>
              <div style={{flex:1}}><p style={{fontFamily:sf,fontSize:16,color:'#999'}}>Estimé</p><p style={{fontFamily:sf,fontSize:26,fontWeight:300}}>{prevCA.predicted}€</p></div>
            </div>
            ...
            <p style={{fontFamily:sf,fontSize:16,color:prevCA.predicted>=prevCA.avg?'#4ADE80':'#F59E0B',fontWeight:500}}>{prevCA.predicted>=prevCA.avg?'✓ En bonne voie':'⚠️ En dessous de ta moyenne — lance une promo !'}</p></>}
          </div>

          <div style={card}>
            <p style={sty}>Jours les plus fréquentés</p>
            <div style={{display:'flex',gap:4,alignItems:'flex-end',height:70}}>
              {busyDays.map((d,i)=>(<div key={i} style={{flex:1,textAlign:'center'}}><div style={{background:d.count>0?tc:'#F0F0EC',height:Math.max((d.count/maxDay)*55,3),borderRadius:2,marginBottom:3}}/><p style={{fontFamily:sf,fontSize:15,color:'#999'}}>{d.name}</p></div>))}
            </div>
          </div>
          ...
          {busyHours.length>0&&<div style={card}>
            ...
            {busyHours.map((h,i)=>(<div key={i} style={{display:'flex',alignItems:'center',gap:6,marginTop:5}}>
              <span style={{fontFamily:sf,fontSize:16,fontWeight:600,width:30}}>{h.hour}</span>
              ...
              <span style={{fontFamily:sf,fontSize:15,color:'#999',width:20}}>{h.count}</span>
            </div>))}
          </div>}

          <div style={{...card,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            <div><p style={{...sty,marginBottom:3}}>Ton lien de réservation</p><p style={{fontFamily:sf,fontSize:18,fontWeight:500}}>mypresty.com/{salon.slug}</p></div>
        ...
        {tab === 'prestations' && (<div>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
            <h1 style={{fontFamily:ss,fontSize:isMobile?30:42,fontWeight:300}}>Prestations</h1>
          ...
          <div style={{...card,display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
            <div><p style={{fontFamily:sf,fontSize:17,fontWeight:500}}>💰 Tarification dynamique</p><p style={{fontFamily:sf,fontSize:16,color:'#999'}}>Ajuste les prix selon le remplissage</p></div>
          ...
          services.map(s=>(<div key={s.id} style={{...card,marginBottom:-1,display:'flex',alignItems:'center',gap:10}}>
            <div style={{flex:1}}>
              <p style={{fontFamily:sf,fontSize:18,fontWeight:500}}>{s.name}</p>
              <p style={{fontFamily:sf,fontSize:16,color:'#999'}}>{s.duration}min{s.category?` · ${s.category}`:''}</p>
              {s.promo_active&&<span style={{fontFamily:sf,fontSize:15,background:'#FEF3C7',color:'#92400E',padding:'2px 6px',display:'inline-block',marginTop:3}}>🏷️ -{s.promo_percent}% {s.promo_label}</span>}
            </div>
            <div style={{textAlign:'right'}}>
              {s.promo_active?<><span style={{fontFamily:sf,fontSize:16,color:'#999',textDecoration:'line-through'}}>{s.price}€</span><br/><span style={{fontFamily:sf,fontSize:20,fontWeight:600}}>{Math.round(s.price*(1-s.promo_percent/100))}€</span></>:
              <span style={{fontFamily:sf,fontSize:20,fontWeight:500}}>{s.price}€</span>}
            </div>
            <div style={{display:'flex',flexDirection:'column',gap:4,flexShrink:0}}>
              <button onClick={()=>togglePromo(s)} style={{fontFamily:sf,fontSize:15,padding:'3px 8px',background:s.promo_active?'#FEF3C7':'#F5F5F3',border:'none',cursor:'pointer',color:s.promo_active?'#92400E':'#999'}}>🏷️ Promo</button>
              <button onClick={()=>deleteService(s.id)} style={{fontFamily:sf,fontSize:15,padding:'3px 8px',background:'#FEF2F2',border:'none',cursor:'pointer',color:'#EF4444'}}>✕</button>
            </div>
          </div>))}
        </div>)}
        ...
        {tab === 'clients' && !selClient && (<div>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
            <h1 style={{fontFamily:ss,fontSize:isMobile?30:42,fontWeight:300}}>Clients</h1>
          ...
          <div style={{background:tc,color:'#FFF',padding:'14px',marginBottom:12}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <div><p style={{fontFamily:sf,fontSize:16,letterSpacing:2,textTransform:'uppercase',opacity:0.5}}>Programme fidélité</p>
              <p style={{fontFamily:sf,fontSize:18,fontWeight:500,marginTop:3}}>{loyaltyConfig?.active?`✓ ${loyaltyConfig.visits_required} visites = ${loyaltyConfig.reward_text}`:'Désactivé'}</p></div>
          ...
          clients.map(c=>{
            ...
            return(<div key={c.id} onClick={()=>setSelClient(c)} style={{...card,marginBottom:-1,display:'flex',alignItems:'center',gap:10,cursor:'pointer'}}>
              <div style={{position:'relative',flexShrink:0}}>
                <div style={{width:36,height:36,borderRadius:'50%',background:isDanger?'#EF4444':tc,display:'flex',alignItems:'center',justifyContent:'center',color:'#FFF',fontFamily:sf,fontSize:17,fontWeight:600}}>{c.first_name?.[0]||''}{c.last_name?.[0]||''}</div>
              ...
              <div style={{flex:1,minWidth:0}}>
                <p style={{fontFamily:sf,fontSize:18,fontWeight:500}}>{c.first_name} {c.last_name} {isDanger&&<span style={{fontSize:16,color:'#EF4444'}}>⚠️</span>}</p>
                <p style={{fontFamily:sf,fontSize:16,color:'#999'}}>{c.phone||'—'} · {c.total_spent||0}€{ltv>0?` · LTV ${ltv}€`:''}</p>
              </div>
              {loyaltyConfig?.active&&<div style={{textAlign:'center',flexShrink:0}}>
                ...
                <p style={{fontFamily:sf,fontSize:15,fontWeight:600,marginTop:-24,position:'relative'}}>{c.loyalty_visits||0}/{loyaltyConfig.visits_required||10}</p>
              </div>}
        ...
        {tab === 'clients' && selClient && (<div>
          <button onClick={()=>setSelClient(null)} style={{fontFamily:sf,fontSize:16,color:'#999',background:'none',border:'none',cursor:'pointer',marginBottom:10}}>← Retour aux clients</button>
          <div style={{textAlign:'center',marginBottom:14}}>
            <div style={{width:56,height:56,borderRadius:'50%',background:dangerClients.some(d=>d.id===selClient.id)?'#EF4444':tc,display:'inline-flex',alignItems:'center',justifyContent:'center',color:'#FFF',fontFamily:sf,fontSize:28,fontWeight:600}}>{selClient.first_name?.[0]}{selClient.last_name?.[0]}</div>
            <h2 style={{fontFamily:sf,fontSize:23,fontWeight:600,marginTop:8}}>{selClient.first_name} {selClient.last_name}</h2>
            <p style={{fontFamily:sf,fontSize:17,color:'#999'}}>{selClient.phone||''} {selClient.email?`· ${selClient.email}`:''}</p>
          </div>
          ...
          {dangerClients.some(d=>d.id===selClient.id)&&<div style={{background:'#FEF2F2',border:'1px solid #FEE2E2',padding:'12px',marginBottom:10,textAlign:'center'}}>
            <p style={{fontFamily:sf,fontSize:16,fontWeight:600,color:'#EF4444'}}>🔴 Cliente en danger de perte</p>
            <p style={{fontFamily:sf,fontSize:16,color:'#B91C1C',marginTop:2}}>{selClient.total_visits||0} visites · Dernière activité il y a longtemps</p>
            <button style={{marginTop:8,padding:'6px 14px',background:'#EF4444',color:'#FFF',border:'none',fontFamily:sf,fontSize:16,fontWeight:600,letterSpacing:1,textTransform:'uppercase',cursor:'pointer'}}>Envoyer un email de relance</button>
          </div>}
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6,marginBottom:12}}>
            {[{l:'Valeur vie',v:`${Math.round((selClient.total_spent||0)*3.5)}€`,bg:tc,fg:'#FFF'},{l:'Dépensé',v:`${selClient.total_spent||0}€`},{l:'Visites',v:selClient.total_visits||0},{l:'Fidélité',v:`${selClient.loyalty_visits||0}/${loyaltyConfig?.visits_required||10}`}].map((s,i)=>(
              <div key={i} style={{background:s.bg||'#FFF',color:s.fg||'#1A1A1A',border:s.bg?'none':'1px solid #E8E8E4',padding:'12px',textAlign:'center'}}>
                <p style={{fontFamily:sf,fontSize:15,textTransform:'uppercase',letterSpacing:1.5,opacity:0.5,marginBottom:3}}>{s.l}</p>
                <p style={{fontSize:23,fontWeight:300}}>{s.v}</p>
              </div>
            ))}
          </div>

          {selClient.allergies&&<div style={{background:'#FEF2F2',border:'1px solid #FEE2E2',padding:'12px',marginBottom:10}}>
            <p style={{fontFamily:sf,fontSize:16,fontWeight:600,color:'#EF4444'}}>⚠️ Allergie : {selClient.allergies}</p>
          </div>}

          {selClient.notes&&<div style={card}><p style={sty}>Notes privées</p><p style={{fontFamily:sf,fontSize:17,color:'#555',lineHeight:1.6}}>{selClient.notes}</p></div>}
          {selClient.birthday&&<div style={card}><p style={{fontFamily:sf,fontSize:17}}>🎂 Anniversaire : {new Date(selClient.birthday).toLocaleDateString('fr-FR')}</p></div>}
          ...
          <div style={card}>
            ...
            {(()=>{const favs=getClientFavorites(selClient);const maxF=Math.max(...favs.map(f=>f[1]),1);return favs.length>0?favs.map(([s,c],i)=>(
              <div key={i} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'6px 0',borderBottom:i<favs.length-1?'1px solid #F0F0EC':'none'}}>
                <span style={{fontFamily:sf,fontSize:17,fontWeight:500}}>{s}</span>
                <div style={{display:'flex',alignItems:'center',gap:8}}>
                  ...
                  <span style={{fontFamily:sf,fontSize:16,color:'#999'}}>{c}×</span>
                </div>
              </div>
            )):<p style={{fontFamily:sf,fontSize:16,color:'#999'}}>Aucune prestation enregistrée</p>;})()}
          </div>
          ...
          <div style={card}>
            ...
            {(()=>{const hist=getClientHistory(selClient);return hist.length>0?hist.slice(0,10).map(a=>(
              <div key={a.id} style={{display:'flex',alignItems:'center',gap:8,padding:'7px 0',borderBottom:'1px solid #F0F0EC'}}>
                <div style={{width:45,flexShrink:0}}><p style={{fontFamily:sf,fontSize:16,fontWeight:600}}>{a.date?.slice(5)}</p><p style={{fontFamily:sf,fontSize:15,color:'#BBB'}}>{a.time}</p></div>
                <div style={{flex:1}}><p style={{fontFamily:sf,fontSize:17,fontWeight:500}}>{a.service_name}</p><p style={{fontFamily:sf,fontSize:15,color:'#999'}}>{a.team_member||''}</p></div>
                <span style={{fontFamily:sf,fontSize:17,fontWeight:600}}>{a.price}€</span>
                <span style={{fontFamily:sf,fontSize:15,padding:'2px 6px',background:a.status==='confirmed'?tc:'#F5F5F3',color:a.status==='confirmed'?'#FFF':'#999'}}>✓</span>
              </div>
            )):<p style={{fontFamily:sf,fontSize:16,color:'#999'}}>Aucun RDV enregistré</p>;})()}
          </div>

          <div style={card}>
            <p style={sty}>Photos avant / après</p>
            <div style={{display:'flex',gap:8}}>
              <div style={{flex:1,height:80,background:'#F0F0EC',display:'flex',alignItems:'center',justifyContent:'center',borderRadius:6}}><span style={{fontFamily:sf,fontSize:16,color:'#999'}}>📷 Avant</span></div>
              <div style={{flex:1,height:80,background:'#F0F0EC',display:'flex',alignItems:'center',justifyContent:'center',borderRadius:6}}><span style={{fontFamily:sf,fontSize:16,color:'#999'}}>📷 Après</span></div>
            </div>
          </div>
        </div>)}
        ...
        {tab === 'equipe' && !selMember && (<div>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
            <h1 style={{fontFamily:ss,fontSize:isMobile?30:42,fontWeight:300}}>Équipe</h1>
          ...
          team.map(m=>{
            ...
            return(<div key={m.id} onClick={()=>setSelMember(m)} style={{...card,marginBottom:-1,cursor:'pointer'}}>
              <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:8}}>
                <div style={{width:38,height:38,borderRadius:'50%',background:m.color||tc,display:'flex',alignItems:'center',justifyContent:'center',color:'#FFF',fontFamily:sf,fontSize:19,fontWeight:600,flexShrink:0}}>{m.name?.[0]||'?'}</div>
                <div style={{flex:1}}><p style={{fontFamily:sf,fontSize:18,fontWeight:500}}>{m.name}</p><p style={{fontFamily:sf,fontSize:16,color:'#999'}}>{m.role||'—'} · {m.commission_percent||0}% commission</p></div>
                <span style={{color:'#CCC',fontSize:18}}>›</span>
              </div>
              {m.target_monthly>0&&<div style={{display:'flex',alignItems:'center',gap:6}}>
                <p style={{fontFamily:sf,fontSize:15,color:'#999',width:55,flexShrink:0}}>Objectif</p>
                ...
                <span style={{fontFamily:sf,fontSize:16,fontWeight:600,flexShrink:0}}>{m.current_month_ca||0}€/{m.target_monthly}€</span>
              </div>}
              {progress>=100&&<p style={{fontFamily:sf,fontSize:16,color:'#4ADE80',marginTop:4,fontWeight:500}}>✓ Objectif atteint !</p>}
            </div>);})}
        </div>)}

        {/* ═══ ESPACE PRIVÉ MEMBRE ═══ */}
        {tab === 'equipe' && selMember && (<div>
          <button onClick={()=>setSelMember(null)} style={{fontFamily:sf,fontSize:16,color:'#999',background:'none',border:'none',cursor:'pointer',marginBottom:10}}>← Retour à l'équipe</button>
          <div style={{textAlign:'center',marginBottom:14}}>
            <div style={{width:56,height:56,borderRadius:'50%',background:selMember.color||tc,display:'inline-flex',alignItems:'center',justifyContent:'center',color:'#FFF',fontFamily:sf,fontSize:28,fontWeight:600}}>{selMember.name?.[0]}</div>
            <h2 style={{fontFamily:sf,fontSize:23,fontWeight:600,marginTop:8}}>{selMember.name}</h2>
            <p style={{fontFamily:sf,fontSize:17,color:'#999'}}>{selMember.role}</p>
          </div>

          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6,marginBottom:12}}>
            <div style={{background:selMember.color||tc,color:'#FFF',padding:'14px',textAlign:'center'}}><p style={{fontFamily:sf,fontSize:15,textTransform:'uppercase',letterSpacing:1.5,opacity:0.6}}>CA ce mois</p><p style={{fontSize:28,fontWeight:300}}>{selMember.current_month_ca||0}€</p></div>
            <div style={{background:'#FFF',border:'1px solid #E8E8E4',padding:'14px',textAlign:'center'}}><p style={{fontFamily:sf,fontSize:15,textTransform:'uppercase',letterSpacing:1.5,color:'#999'}}>Commission</p><p style={{fontSize:28,fontWeight:300}}>{Math.round((selMember.current_month_ca||0)*(selMember.commission_percent||0)/100)}€</p></div>
          </div>

          <div style={card}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
              <p style={sty}>🧪 Formules & recettes</p>
              <button onClick={()=>setShowForm(showForm==='formula'?null:'formula')} style={{fontFamily:sf,fontSize:15,color:tc,background:'none',border:'none',cursor:'pointer',fontWeight:600}}>+ Ajouter</button>
            </div>
            ...
            {(()=>{const formulas=JSON.parse(selMember.formulas_json||'[]');return formulas.length>0?formulas.map((f,i)=>(
              <div key={i} style={{padding:'8px 0',borderBottom:i<formulas.length-1?'1px solid #F0F0EC':'none'}}>
                <div style={{display:'flex',justifyContent:'space-between'}}><p style={{fontFamily:sf,fontSize:17,fontWeight:500}}>{f.client}</p><span style={{fontFamily:sf,fontSize:15,color:'#BBB'}}>{f.date}</span></div>
                <p style={{fontFamily:sf,fontSize:16,color:'#666',marginTop:2}}>{f.formula}</p>
              </div>
            )):<p style={{fontFamily:sf,fontSize:16,color:'#999'}}>Aucune formule. Ajoute tes recettes couleur ici !</p>;})()}
          </div>

          <div style={card}>
            <p style={sty}>📝 Notes personnelles</p>
            <p style={{fontFamily:sf,fontSize:17,color:'#666',lineHeight:1.6}}>{selMember.notes||'Aucune note.'}</p>
          </div>

          <div style={card}>
            <p style={sty}>🎤 Note vocale rapide</p>
            <div style={{display:'flex',alignItems:'center',gap:10}}>
              <div style={{width:44,height:44,borderRadius:'50%',background:selMember.color||tc,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer'}}><span style={{fontSize:23,color:'#FFF'}}>🎙️</span></div>
              <p style={{fontFamily:sf,fontSize:16,color:'#999'}}>Appuie pour dicter une formule ou une note.<br/>L'IA transcrit et classe automatiquement.</p>
            </div>
          </div>

          <button onClick={()=>deleteMember(selMember.id)} style={{fontFamily:sf,fontSize:16,color:'#EF4444',background:'none',border:'1px solid #FEE2E2',padding:'8px 16px',cursor:'pointer',width:'100%',marginTop:8}}>Supprimer ce membre</button>
        </div>)}

        {/* ═══ RDV ═══ */}
        {tab === 'rdv' && (<div>
          <h1 style={{fontFamily:ss,fontSize:isMobile?30:42,fontWeight:300,marginBottom:10}}>Rendez-vous</h1>

          <div style={{display:'flex',gap:6,marginBottom:12}}>
            <div style={{...card,flex:1,display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:0}}>
              <div><p style={{fontFamily:sf,fontSize:16,fontWeight:500}}>Créneaux auto</p><p style={{fontFamily:sf,fontSize:15,color:'#999'}}>Gap: {salon.slot_gap||30}min</p></div>
              <Tgl on={salon.auto_slots!==false} fn={()=>saveSalonSettings({auto_slots:salon.auto_slots===false})}/>
            </div>
            <div style={{...card,flex:1,display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:0}}>
              <div><p style={{fontFamily:sf,fontSize:16,fontWeight:500}}>Waitlist</p><p style={{fontFamily:sf,fontSize:15,color:'#999'}}>{waitlist.length} en att.</p></div>
          ...
          {waitlist.length>0&&<div style={{background:'#F5F3FF',border:'1px solid #E9D5FF',padding:'12px',marginBottom:12}}>
            <p style={{fontFamily:sf,fontSize:16,fontWeight:600,color:'#7C3AED',marginBottom:6}}>📋 Liste d'attente</p>
            {waitlist.map(w=>(<div key={w.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'4px 0'}}>
              <span style={{fontFamily:sf,fontSize:16}}>{w.client_name} — {w.service_name}</span>
              <span style={{fontFamily:sf,fontSize:16,color:'#999'}}>{w.preferred_date} {w.preferred_time}</span>
            </div>))}
          </div>}
          ...
          appointments.slice(0,20).map(a=>(<div key={a.id} style={{...card,marginBottom:-1,display:'flex',alignItems:'center',gap:6}}>
            <div style={{width:42,flexShrink:0}}><p style={{fontFamily:sf,fontSize:17,fontWeight:600}}>{a.time||'—'}</p><p style={{fontFamily:sf,fontSize:15,color:'#BBB'}}>{a.date?.slice(5)}</p></div>
            <div style={{flex:1,minWidth:0}}><p style={{fontFamily:sf,fontSize:17,fontWeight:500}}>{a.client_name}</p><p style={{fontFamily:sf,fontSize:15,color:'#999'}}>{a.service_name} {a.team_member?`· ${a.team_member}`:''}</p></div>
            {a.deposit_paid&&<span style={{fontFamily:sf,fontSize:15,background:'#F0FDF4',color:'#10B981',padding:'2px 6px'}}>💳</span>}
            <span style={{fontFamily:sf,fontSize:18,fontWeight:600,flexShrink:0}}>{a.price}€</span>
            <span style={{fontFamily:sf,fontSize:15,padding:'3px 7px',background:a.status==='confirmed'?tc:'transparent',color:a.status==='confirmed'?'#FFF':'#999',border:a.status==='confirmed'?'none':'1px solid #DDD',flexShrink:0}}>{a.status==='confirmed'?'✓':'...'}</span>
          </div>))}
        </div>)}
        ...
        {tab === 'avis' && (<div>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
            <h1 style={{fontFamily:ss,fontSize:isMobile?30:42,fontWeight:300}}>Avis clients</h1>
            <div style={{textAlign:'right'}}><p style={{fontFamily:sf,fontSize:34,fontWeight:300}}>{avgRating}<span style={{fontSize:18,color:'#999'}}>/5</span></p><p style={{fontSize:17,color:'#F59E0B'}}>{avgRating>=4.5?'★★★★★':avgRating>=3.5?'★★★★☆':'★★★☆☆'}</p></div>
          </div>
          ...
          reviews.map(r=>(<div key={r.id} style={card}>
            <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
              <div><p style={{fontFamily:sf,fontSize:18,fontWeight:500}}>{r.client_name}</p><p style={{fontSize:16,color:'#F59E0B',marginTop:2}}>{stars(r.rating)}</p></div>
              <span style={{fontFamily:sf,fontSize:16,color:'#BBB'}}>{new Date(r.created_at).toLocaleDateString('fr-FR')}</span>
            </div>
            <p style={{fontFamily:sf,fontSize:17,color:'#555',lineHeight:1.6,marginTop:4}}>{r.comment}</p>
            {r.service_name&&<p style={{fontFamily:sf,fontSize:16,color:'#999',marginTop:4}}>Prestation : {r.service_name}</p>}
            {r.reply?<div style={{background:'#F8F8F6',padding:'8px 10px',marginTop:8}}>
              <p style={{fontFamily:sf,fontSize:16,fontWeight:600,color:tc}}>Votre réponse :</p>
              <p style={{fontFamily:sf,fontSize:16,color:'#666',marginTop:2}}>{r.reply}</p>
            </div>:
            ...
            <div onClick={()=>setReplyingTo(r.id)} style={{marginTop:8,padding:'7px',border:`1px dashed ${tc}`,textAlign:'center',cursor:'pointer'}}><span style={{fontFamily:sf,fontSize:16,color:tc,fontWeight:500}}>Répondre</span></div>}
          </div>))}
        </div>)}
        ...
        {tab === 'produits' && (<div>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
            <h1 style={{fontFamily:ss,fontSize:isMobile?30:42,fontWeight:300}}>Produits</h1>
            <button onClick={()=>setShowForm(showForm==='product'?null:'product')} style={btn()}>{showForm==='product'?'✕ Fermer':'+ Ajouter'}</button>
          </div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:6,marginBottom:12}}>
            <div style={{background:tc,color:'#FFF',padding:'14px',textAlign:'center'}}><p style={{fontFamily:sf,fontSize:15,textTransform:'uppercase',letterSpacing:1.5,opacity:0.5}}>Réf.</p><p style={{fontSize:28,fontWeight:300}}>{products.length}</p></div>
            <div style={{background:'#FFF',border:'1px solid #E8E8E4',padding:'14px',textAlign:'center'}}><p style={{fontFamily:sf,fontSize:15,textTransform:'uppercase',letterSpacing:1.5,color:'#999'}}>Stock</p><p style={{fontSize:28,fontWeight:300}}>{products.reduce((s,p)=>s+(p.stock||0),0)}</p></div>
            <div style={{background:'#FFF',border:'1px solid #E8E8E4',padding:'14px',textAlign:'center'}}><p style={{fontFamily:sf,fontSize:15,textTransform:'uppercase',letterSpacing:1.5,color:'#999'}}>Valeur</p><p style={{fontSize:28,fontWeight:300}}>{products.reduce((s,p)=>s+(p.price||0)*(p.stock||0),0)}€</p></div>
          </div>
          ...
          products.map(p=>(<div key={p.id} style={{...card,marginBottom:-1,display:'flex',alignItems:'center',gap:10}}>
            <div style={{flex:1}}><p style={{fontFamily:sf,fontSize:17,fontWeight:500}}>{p.name}</p><p style={{fontFamily:sf,fontSize:16,color:'#999'}}>{p.brand||''} {p.category?`· ${p.category}`:''}</p></div>
            <div style={{textAlign:'right',flexShrink:0}}><p style={{fontFamily:sf,fontSize:18,fontWeight:600}}>{p.price}€</p><p style={{fontFamily:sf,fontSize:16,color:p.stock<6?'#EF4444':'#999'}}>{p.stock<6?'⚠️ ':''}{p.stock||0} stock</p></div>
            <button onClick={()=>deleteProduct(p.id)} style={{fontFamily:sf,fontSize:15,padding:'3px 8px',background:'#FEF2F2',border:'none',cursor:'pointer',color:'#EF4444',flexShrink:0}}>✕</button>
          </div>))}
        </div>)}
        ...
        {tab === 'forfaits' && (<div>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
            <h1 style={{fontFamily:ss,fontSize:isMobile?30:42,fontWeight:300}}>Forfaits</h1>
          ...
          packages.map(p=>(<div key={p.id} style={{...card,position:'relative'}}>
            {p.original_price>0&&<div style={{position:'absolute',top:0,right:0,background:'#4ADE80',padding:'3px 8px'}}><span style={{fontFamily:sf,fontSize:15,fontWeight:700}}>-{Math.round((1-p.total_price/p.original_price)*100)}%</span></div>}
            <p style={{fontFamily:sf,fontSize:19,fontWeight:600,marginBottom:6}}>{p.name}</p>
            {p.description&&<p style={{fontFamily:sf,fontSize:16,color:'#777',marginBottom:6}}>{p.description}</p>}
            {(p.services||[]).map((s,i)=>(<span key={i} style={{fontFamily:sf,fontSize:15,background:'#F5F5F3',padding:'3px 8px',marginRight:4,display:'inline-block',marginBottom:4}}>{s}</span>))}
            <div style={{display:'flex',alignItems:'center',gap:8,marginTop:6}}>
              {p.original_price>0&&<span style={{fontFamily:sf,fontSize:17,color:'#999',textDecoration:'line-through'}}>{p.original_price}€</span>}
              <span style={{fontFamily:sf,fontSize:26,fontWeight:600}}>{p.total_price}€</span>
              {p.original_price>0&&<span style={{fontFamily:sf,fontSize:16,color:'#4ADE80',fontWeight:500}}>Économise {p.original_price-p.total_price}€</span>}
            </div>
            <button onClick={()=>deletePackage(p.id)} style={{fontFamily:sf,fontSize:15,color:'#EF4444',background:'none',border:'none',cursor:'pointer',marginTop:6}}>Supprimer</button>
          </div>))}
        ...
        {/* ═══ PARRAINAGE ═══ */}
        {tab === 'parrainage' && (<div>
          <h1 style={{fontFamily:ss,fontSize:isMobile?30:42,fontWeight:300,marginBottom:10}}>Parrainage</h1>
          <div style={{background:tc,color:'#FFF',padding:'16px',marginBottom:12}}>
            <p style={{fontFamily:sf,fontSize:16,letterSpacing:2,textTransform:'uppercase',opacity:0.5}}>Programme de parrainage</p>
            <div style={{display:'flex',gap:14,marginTop:8}}>
              <div><p style={{fontSize:38,fontWeight:300}}>{clients.filter(c=>c.referred_by).length}</p><p style={{fontFamily:sf,fontSize:16,opacity:0.6}}>Filleules</p></div>
              <div><p style={{fontSize:38,fontWeight:300}}>{clients.filter(c=>c.referred_by).reduce((s,c)=>s+(c.total_spent||0),0)}€</p><p style={{fontFamily:sf,fontSize:16,opacity:0.6}}>CA généré</p></div>
            </div>
          </div>
          <div style={card}>
            <p style={sty}>Règles actives</p>
            <p style={{fontFamily:sf,fontSize:17,color:'#555'}}><strong>Marraine :</strong> 10€ de crédit par filleule</p>
            <p style={{fontFamily:sf,fontSize:17,color:'#555',marginTop:3}}><strong>Filleule :</strong> -20% sur le 1er RDV</p>
          </div>
          ...
          {clients.filter(c=>c.referral_count>0).sort((a,b)=>(b.referral_count||0)-(a.referral_count||0)).length>0?
            clients.filter(c=>c.referral_count>0).sort((a,b)=>(b.referral_count||0)-(a.referral_count||0)).map((c,i)=>(
              <div key={c.id} style={{...card,marginBottom:-1,display:'flex',alignItems:'center',gap:10}}>
                <span style={{fontFamily:sf,fontSize:20,fontWeight:700,color:i===0?'#F59E0B':i===1?'#999':'#CCC',width:24}}>{i+1}.</span>
                <div style={{width:30,height:30,borderRadius:'50%',background:tc,display:'flex',alignItems:'center',justifyContent:'center',color:'#FFF',fontFamily:sf,fontSize:16,fontWeight:600}}>{c.first_name?.[0]}{c.last_name?.[0]}</div>
                <div style={{flex:1}}><p style={{fontFamily:sf,fontSize:17,fontWeight:500}}>{c.first_name} {c.last_name}</p><p style={{fontFamily:sf,fontSize:16,color:'#999'}}>{c.referral_count} filleules</p></div>
        ...
        {tab === 'com' && (<div>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
            <h1 style={{fontFamily:ss,fontSize:isMobile?30:42,fontWeight:300}}>Communication</h1>
          ...
          messages.map(m=>(<div key={m.id} style={card}>
            <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
              <p style={{fontFamily:sf,fontSize:18,fontWeight:500}}>{m.title}</p>
              <span style={{fontFamily:sf,fontSize:15,color:'#BBB'}}>{new Date(m.sent_at).toLocaleDateString('fr-FR')}</span>
            </div>
            <p style={{fontFamily:sf,fontSize:16,color:'#666',lineHeight:1.6}}>{m.content}</p>
            <p style={{fontFamily:sf,fontSize:15,color:'#BBB',marginTop:6,letterSpacing:1,textTransform:'uppercase'}}>Envoyé à {clients.length} client(e)s</p>
          </div>))}
        </div>)}

        {/* ═══ ASSISTANT IA ═══ */}
        {tab === 'ia' && (<div>
          <h1 style={{fontFamily:ss,fontSize:isMobile?30:42,fontWeight:300,marginBottom:4}}>Assistant IA</h1>
          <p style={{fontFamily:sf,fontSize:16,color:'#999',marginBottom:12}}>Pose une question sur ton salon. L'IA analyse tes données en temps réel.</p>
          <div style={{display:'flex',flexWrap:'wrap',gap:5,marginBottom:14}}>
            {['Comment va mon mois ?','Qui recontacter ?','Quel créneau est faible ?','Quelle promo lancer ?'].map(q=>(<button key={q} onClick={()=>handleAi(q)} style={{padding:'7px 12px',background:'#FFF',border:'1px solid #E8E8E4',fontFamily:sf,fontSize:16,cursor:'pointer',color:'#555'}}>{q}</button>))}
          </div>
          <div style={{background:'#FFF',border:'1px solid #E8E8E4',minHeight:280,maxHeight:400,overflowY:'auto',marginBottom:10,padding:12,scrollbarWidth:'none'}}>
            {aiChat.length===0&&<div style={{textAlign:'center',padding:'50px 14px'}}><p style={{fontSize:42}}>🧠</p><p style={{fontFamily:sf,fontSize:18,color:'#BBB',marginTop:10}}>Demande-moi n'importe quoi sur ton business !</p></div>}
            {aiChat.map((m,i)=>(<div key={i} style={{marginBottom:12,textAlign:m.role==='user'?'right':'left'}}>
              <div style={{display:'inline-block',maxWidth:'85%',padding:'10px 14px',background:m.role==='user'?tc:'#F5F5F3',color:m.role==='user'?'#FFF':'#333',fontFamily:sf,fontSize:17,lineHeight:1.7,borderRadius:m.role==='user'?'14px 14px 3px 14px':'14px 14px 14px 3px',whiteSpace:'pre-wrap'}}>{m.text}</div>
            </div>))}
            {aiTyping&&<div style={{marginBottom:12}}><div style={{display:'inline-block',padding:'10px 18px',background:'#F5F5F3',borderRadius:14}}><span style={{fontFamily:sf,fontSize:17,color:'#999'}}>🧠 Analyse en cours...</span></div></div>}
          </div>
          <div style={{display:'flex',gap:8}}>
        ...
        {tab === 'salon' && (<div>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
            <h1 style={{fontFamily:ss,fontSize:isMobile?30:42,fontWeight:300}}>Mon salon</h1>
            <button onClick={()=>setShowForm(showForm==='photo'?null:'photo')} style={btn()}>{showForm==='photo'?'✕ Fermer':'+ Photo'}</button>
          </div>

          <div style={{...card,display:'flex',alignItems:'center',gap:12}}>
            <div style={{width:50,height:50,borderRadius:12,background:tc,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}><span style={{color:'#FFF',fontFamily:ss,fontSize:26,fontWeight:600}}>{salon.name?.[0]||'P'}</span></div>
            <div><p style={{fontFamily:sf,fontSize:19,fontWeight:500}}>{salon.name}</p><p style={{fontFamily:sf,fontSize:16,color:'#999'}}>{salon.category||'Beauté'} {salon.city?`· ${salon.city}`:''}</p></div>
          </div>

          <div style={card}>
            <p style={sty}>Description</p>
            <p style={{fontFamily:sf,fontSize:17,color:'#666',lineHeight:1.7}}>{salon.description||'Aucune description.'}</p>
          </div>
          ...
          {photos.length>0&&<>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:4}}>
              {photos.map(p=>(<div key={p.id} style={{position:'relative'}}>
                {p.url?<img src={p.url} alt={p.caption} style={{width:'100%',height:90,objectFit:'cover'}}/>:<div style={{width:'100%',height:90,background:'#F0F0EC',display:'flex',alignItems:'center',justifyContent:'center'}}><span style={{fontSize:20,opacity:0.3}}>📷</span></div>}
                <p style={{fontFamily:sf,fontSize:15,padding:'3px 4px',color:'#777',background:'#FFF',border:'1px solid #E8E8E4',borderTop:'none'}}>{p.caption||'Photo'}</p>
                <button onClick={()=>deletePhoto(p.id)} style={{position:'absolute',top:2,right:2,width:18,height:18,background:'rgba(0,0,0,0.5)',color:'#FFF',border:'none',fontSize:16,cursor:'pointer',borderRadius:'50%'}}>✕</button>
              </div>))}
            </div>
          </>}
        </div>)}
        ...
        {/* ═══ RÉGLAGES ═══ */}
        {tab === 'reglages' && (<div>
          <h1 style={{fontFamily:ss,fontSize:isMobile?30:42,fontWeight:300,marginBottom:14}}>Réglages</h1>

          <div style={{background:tc,color:'#FFF',padding:'20px',marginBottom:10}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
              <div>
                <p style={{fontFamily:sf,fontSize:15,letterSpacing:2,textTransform:'uppercase',opacity:0.5}}>Abonnement</p>
                <p style={{fontFamily:sf,fontSize:19,fontWeight:500,marginTop:4}}>MY PRESTY — 19€/mois</p>
                {salon.subscription_status==='trial'&&salon.trial_end&&(()=>{
                  const daysLeft=Math.max(0,Math.ceil((new Date(salon.trial_end).getTime()-Date.now())/(1000*60*60*24)));
                  const endDate=new Date(salon.trial_end).toLocaleDateString('fr-FR',{day:'numeric',month:'long',year:'numeric'});
                  return <><p style={{fontFamily:sf,fontSize:17,marginTop:6,fontWeight:500}}>✦ Essai gratuit en cours</p>
                  <p style={{fontFamily:sf,fontSize:16,opacity:0.7,marginTop:3}}>{daysLeft} jours restants — jusqu'au {endDate}</p>
                  <div style={{background:'rgba(255,255,255,0.15)',height:8,borderRadius:4,marginTop:8,width:'100%'}}><div style={{background:daysLeft>30?'#4ADE80':daysLeft>7?'#F59E0B':'#EF4444',height:8,borderRadius:4,width:`${Math.max(5,(daysLeft/90)*100)}%`}}/></div></>;
                })()}
                {salon.subscription_status==='active'&&<p style={{fontFamily:sf,fontSize:17,marginTop:6,color:'#4ADE80',fontWeight:500}}>✓ Abonnement actif</p>}
                {(!salon.subscription_status||salon.subscription_status==='none'||salon.subscription_status==='expired')&&<p style={{fontFamily:sf,fontSize:17,marginTop:6,color:'#EF4444',fontWeight:500}}>Pas d'abonnement actif</p>}
              </div>
              <span style={{fontFamily:sf,fontSize:15,padding:'5px 12px',border:`1px solid ${salon.subscription_status==='active'||salon.subscription_status==='trial'?'rgba(74,222,128,0.5)':'rgba(239,68,68,0.5)'}`,color:salon.subscription_status==='active'||salon.subscription_status==='trial'?'#4ADE80':'#EF4444',letterSpacing:1,textTransform:'uppercase',flexShrink:0}}>
                {salon.subscription_status==='active'?'✓ Pro':salon.subscription_status==='trial'?'✦ Essai':'✕'}
              </span>
            </div>
            {salon.subscription_status!=='active'&&<button onClick={async()=>{const res=await fetch('/api/stripe/checkout',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:user.email,salonId:salon.id})});const data=await res.json();if(data.url)window.location.href=data.url;}} style={{marginTop:12,padding:'10px 22px',background:'rgba(255,255,255,0.15)',border:'1px solid rgba(255,255,255,0.3)',color:'#FFF',fontFamily:sf,fontSize:15,fontWeight:600,letterSpacing:1.5,textTransform:'uppercase',cursor:'pointer'}}>Passer au plan Pro — 19€/mois</button>}
          </div>

          <div style={card}>
            <p style={sty}>Paiement en ligne</p>
            {salon.stripe_account_id?
              <p style={{fontFamily:sf,fontSize:17,color:'#4ADE80',fontWeight:500}}>✓ Stripe connecté</p>:
              <button onClick={handleStripeConnect} style={{padding:'10px 20px',background:'#635BFF',color:'#FFF',border:'none',fontFamily:sf,fontSize:16,fontWeight:600,letterSpacing:1.5,textTransform:'uppercase',cursor:'pointer'}}>Connecter Stripe 💳</button>}
          </div>
          ...
          <div style={card}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <div><p style={{fontFamily:sf,fontSize:18,fontWeight:500}}>🛡️ Protection no-show</p><p style={{fontFamily:sf,fontSize:16,color:'#999'}}>Acompte de {salon.deposit_percent||30}% obligatoire</p></div>
          ...
          <div style={card}>
            ...
            <p style={{fontFamily:sf,fontSize:17,color:'#555'}}>Annulation possible jusqu'à <strong>{salon.cancellation_hours||24}h avant</strong></p>
            <p style={{fontFamily:sf,fontSize:16,color:'#999',marginTop:3}}>Frais d'annulation tardive : {salon.cancellation_fee||50}% du prix</p>
          </div>

          <div style={card}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <div><p style={{fontFamily:sf,fontSize:18,fontWeight:500}}>✉️ Rappels email</p><p style={{fontFamily:sf,fontSize:16,color:'#999'}}>Rappel automatique avant chaque RDV</p></div>
          ...
          <div style={card}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <div><p style={{fontFamily:sf,fontSize:18,fontWeight:500}}>💰 Tarification dynamique</p><p style={{fontFamily:sf,fontSize:16,color:'#999'}}>Ajuste les prix selon les créneaux</p></div>
              <Tgl on={dynPricing} fn={()=>{setDynPricing(!dynPricing);saveSalonSettings({dyn_pricing_enabled:!dynPricing});}}/>
            </div>
          </div>
          ...
          <div style={card}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <div><p style={{fontFamily:sf,fontSize:18,fontWeight:500}}>🎰 Roue de la chance</p><p style={{fontFamily:sf,fontSize:16,color:'#999'}}>Après chaque paiement client</p></div>
          ...
          <div style={card}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <div><p style={{fontFamily:sf,fontSize:18,fontWeight:500}}>🧲 Carte cadeau digitale</p><p style={{fontFamily:sf,fontSize:16,color:'#999'}}>Visible sur ta page publique</p></div>
          ...
          <div style={card}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <div><p style={{fontFamily:sf,fontSize:18,fontWeight:500}}>📋 Liste d'attente</p><p style={{fontFamily:sf,fontSize:16,color:'#999'}}>Quand les créneaux sont pleins</p></div>
          ...
          <div style={card}>
            ...
            <div style={{display:'flex',gap:10,alignItems:'center'}}>
              <div style={{padding:'8px 12px',border:'1px solid #E8E8E4',fontFamily:sf,fontSize:17}}>{salon.opening_time||'9h00'}</div>
              <span style={{color:'#CCC'}}>→</span>
              <div style={{padding:'8px 12px',border:'1px solid #E8E8E4',fontFamily:sf,fontSize:17}}>{salon.closing_time||'19h00'}</div>
            </div>
          </div>

          <div style={card}>
            <p style={sty}>Lien de réservation</p>
            <p style={{fontFamily:sf,fontSize:18,fontWeight:500}}>mypresty.com/{salon.slug}</p>
          </div>

          <div style={card}>
            <p style={sty}>Compte</p>
            <p style={{fontFamily:sf,fontSize:17,color:'#777'}}>{user?.email}</p>
          </div>

          <button onClick={async()=>{await supabase.auth.signOut();router.push('/connexion');}} style={{width:'100%',padding:'12px',background:'none',border:'1px solid #EF4444',color:'#EF4444',fontFamily:sf,fontSize:16,fontWeight:600,letterSpacing:1.5,textTransform:'uppercase',cursor:'pointer',marginTop:8}}>Se déconnecter</button>
        </div>)}
